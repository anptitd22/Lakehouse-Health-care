...
DISEASES_PATH = f"s3a://lakehouse/bronze/health_care_raw/ingest_dt={date.today().isoformat()}/diseases*.parquet"
CATALOG = "local"
DB = "silver"
TABLE = "diseases"
...
def diseases_bronze_to_silver():
    ...
    try:
        ...
        diseases_raw = (
            spark.read.parquet(DISEASES_PATH)
        )
        ...
        .withColumn(
            ...
            , struct(
                ...
                , col('disease_name').cast('string').alias('disease_name')
                , col('diagnosis_date').cast('string').alias('diagnosis_date')
        ...
        (df.select(col("diseases.*")).coalesce(6)
        ...
        # logger.info(f'Created {CATALOG}.{DB}.diseases successfully.')
        print(f'Created {CATALOG}.{DB}.{TABLE}.diseases successfully.')
